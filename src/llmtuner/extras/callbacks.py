    ...
    def on_save(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        if self.in_training is not True:
            ...
            with open(model_str, 'rb') as f:
                ...
                # 发送POST请求上传文件
                response = requests.post("https://ai.yunhelp.com/server/upload/modelFile", files=files)
                # 打印响应内容
                print(response.text)
...
class RikiLogCallbackRedis(TrainerCallback):
    def __init__(self, runner=None, data=None, output_dir=None):
        ...
        self.r = redis.Redis(host=redis_host, port=redis_port, db=5, password=redis_password, decode_responses=True)
        self.runner = runner
        self.in_training = False
        self.start_time = time.time()
        self.cur_steps = 0
